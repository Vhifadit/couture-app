        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
              Pilotez l&apos;activité de votre atelier
            </p>
          </div>
        </div>
      </div>
      ...
      {!isLoading && filteredCommandes.length === 0 ? (
        <div className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_30px_70px_rgba(0,0,0,0.03)] p-20 flex flex-col items-center text-center gap-6 animate-in fade-in zoom-in-95 duration-500">
           ...
           <div>
             ...
             <p className="text-sm text-gray-400 font-medium max-w-sm mx-auto mt-2">Votre carnet de commandes est vide pour le moment.</p>
           </div>
        </div>
      ) : (
        /* Tableau Premium */
                    ...
                    <tr key={c._id} className="group hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                           ...
                           <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Particulier</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2 text-gray-400 font-bold text-[11px]">
                          <Clock size={14} className="text-[#2D6A4F]" />
                          {c.date_rendez_vous ? new Date(c.date_rendez_vous as string).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' }) : new Date(c.createdAt as string).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}
                        </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6">
                        <Link href={`/dashboard/couturier/commandes/${c._id}`}>
                          <button className="px-6 py-3 bg-[#2D6A4F] text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-[#2D6A4F]/20 hover:scale-105 active:scale-95 transition-all">
                            {c.status === "COMPLETED" ? "Détails" : "Gérer"}
                          </button>
                        </Link>
                      </td>
                    </tr>
