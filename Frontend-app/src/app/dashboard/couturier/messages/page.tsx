  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-[#2D6A4F] animate-pulse">Chargement de votre messagerie...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-10 h-[calc(100vh-140px)]">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
              Gérez votre relation client en direct
            </p>
          </div>
        </div>
      </div>
      ...
      {conversations.length === 0 ? (
        <div className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_30px_70px_rgba(0,0,0,0.03)] p-20 flex flex-col items-center text-center gap-6 animate-in fade-in zoom-in-95 duration-500 flex-1 justify-center">
           ...
           <div>
             <h2 className="text-2xl font-black text-gray-900 tracking-tight">Aucune discussion</h2>
             <p className="text-sm text-gray-400 font-medium max-w-sm mx-auto mt-2">Votre messagerie est vide. Les nouveaux messages apparaîtront dès qu&apos;un client vous contactera.</p>
           </div>
        </div>
      ) : (
          ...
          <div className="w-full md:w-80 lg:w-96 border-r border-gray-50 flex flex-col shrink-0">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar py-2">
              {conversations.map((conv) => {
                const clientName = typeof conv.client_id === 'object' ? conv.client_id?.name : 'Client';
                return (
                  <button
                    key={conv._id}
          ...
          <div className="flex flex-col flex-1 min-w-0 bg-[#FAF9F6]/30">
            {selectedConv ? (
              <>
                  ...
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <p className="text-sm font-black text-[#2D6A4F] uppercase tracking-tight">
                          {typeof selectedConv.client_id === 'object' ? selectedConv.client_id?.name : 'Client'}
                        </p>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                  {isLoadingMessages ? (
                    ...
                    <>
                      {messages.map((m) => {
                        const isMe = m.expediteur_id === selectedConv.couturier_id;
                        return (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center p-10 gap-4">
                ...
                <div>
                   <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Sélectionnez un client</p>
                   <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest mt-1">Gérez vos demandes en direct</p>
                </div>
              </div>
            )}
          </div>
