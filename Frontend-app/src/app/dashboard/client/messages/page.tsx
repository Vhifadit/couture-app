  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm mt-1 text-ardoise-light">Vos échanges avec les couturiers</p>
      </div>
      ...
      {conversations.length === 0 ? (
        <div className="text-center py-10 text-gray-500">
          ...
          <p className="text-sm mt-2">Contactez un couturier pour commencer à discuter.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex justify-between items-center gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="avatar bg-em shrink-0">
                      {conv.couturierName?.[0] || "C"}
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-em truncate">{conv.couturierName}</p>
            ...
            {selectedConv ? (
              <>
                <div className="p-4 border-b-pierre">
                  <p className="font-semibold text-em">{selectedConv.couturierName}</p>
                  <p className="text-xs text-em-light">● En ligne</p>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                  {isLoadingMessages ? (
                    ...
                    <>
                      {messages.map((m) => (
                        <div key={m._id} className={`flex ${m.expediteur_role === 'client' ? "justify-end" : "justify-start"}`}>
                          <div className={m.expediteur_role === 'client' ? "chat-bubble-me" : "chat-bubble-other"}>
                            <p>{m.contenu}</p>
