...
// ✅ NOUVEAU: Obtenir tarifs d'un couturier (pour formulaire commande)
const getTarifsById = async (req, res) => {
  try {
    const { id } = req.params;
    
    const couturier = await Couturier.findById(id).select('tarifs nom_marque services');
    ...
    res.json({ 
      ...
      tarifs: couturier.tarifs || {},
      services: couturier.services || []
    });
  } catch (error) {
    res.status(500).json({ message: 'Erreur serveur', error: error.message });
  }
...
// ✅ NOUVEAU: Obtenir les meilleurs couturiers (pour la page d'accueil)
// - Si des couturiers ont des notes : les 3 meilleurs par note
// - Si aucun n'a de notes : les 3 premiers inscrits
const getTopCouturiers = async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 3;
    
    // D'abord, essayer de trouver les couturiers avec des notes
    let couturiers = await Couturier.find({ 
      disponibilite: true,
      'stats.note_moyenne': { $gt: 0 }
    })
      .populate('user_id', 'name email')
    ...
    // Si aucun couturier n'a de notes, prendre les 3 premiers inscrits
    if (couturiers.length === 0) {
      couturiers = await Couturier.find({ disponibilite: true })
        .populate('user_id', 'name email')
        .sort({ createdAt: 1 })  // Les premiers inscrits
        .limit(limit)
        .select('-__v');
    }
